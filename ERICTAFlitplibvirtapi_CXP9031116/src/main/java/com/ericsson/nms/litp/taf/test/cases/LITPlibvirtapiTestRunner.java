...
public class LITPlibvirtapiTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPlibvirtapiTestRunner.class);
    ...
    @TestId(id = "CXP9031116-1", title = "Upgrade ERIClitplibvirtapi rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitplibvirtapiRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("libvirtapi");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031116-2", title = "Run python test cases for ERIClitplibvirtapi package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitplibvirtapiTests() {
     ...
     * @PRE Execution of test {@link #runERIClitplibvirtapiTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031116-3", title = "Parse xml outputs from python tests")
